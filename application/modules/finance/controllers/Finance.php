...
class Finance extends MX_Controller
{
    ...
    function getPayment()
    {
        ...
        foreach ($data['payments'] as $payment) {
            ...
            $options2 = '<a class="btn btn-warning btn-sm invoicebutton mt-1" title="' . lang('invoice') . '" href="finance/invoice?id=' . $payment->id . '"><i class="fa fa-file-invoice"></i> ' . lang('invoice') . '</a>';
            $options4 = '<a class="btn btn-info btn-sm invoicebutton mr-1 mt-1" title="' . lang('print') . '" style="color: #fff;" href="finance/printInvoice?id=' . $payment->id . '"target="_blank"> <i class="fa fa-print"></i> ' . lang('print') . '</a>';
            if ($this->ion_auth->in_group(array('admin', 'Accountant'))) {
                if ($payment->payment_from == 'payment' || empty($payment->payment_from)) {
                    if (count($lab_reports_previous) == count($lab_pending) || empty($lab_reports_previous)) {
            ...
            <div class="btn-group">
                ...
                <ul class="dropdown-menu">
                    ' . ($options1 ? '<li><a href="' . $href . '"><i class="fa fa-edit"></i> ' . lang('edit') . ' </a></li>' : '') . '
                    ' . ($options2 ? '<li><a href="' . site_url("finance/invoice?id=" . $payment->id) . '"><i class="fa fa-file-invoice"></i> ' . lang('invoice') . ' </a></li>' : '') . '
                    ' . ($options4 ? '<li><a href="' . site_url("finance/printInvoice?id=" . $payment->id) . '" target="_blank"><i class="fa fa-print"></i> ' . lang('print') . ' </a></li>' : '') . '
    ...
    function sendInvoice()
    {
        ...
        $logo = $this->settings_model->getSettings()->logo;
        if(!empty($logo)){
            $logo_invoice = $logo;
        }else{
            $logo_invoice = 'uploads/favicon.png';
        }
        ...
        $header = '<div id="invoice_header" style="width:100%;">
                       <table class="info_rer">
                           <tr class="tr_info">
                               ...
                               <td id="first_td" style="">
                                   <img class="img_class_logo" alt="" src="' .  $logo_invoice . '" width="120">
                                   <br>
    ...
    function getDuePayment()
    {
        ...
        foreach ($data['payments'] as $payment) {
            ...
            $options2 = '<a class="btn btn-warning btn-sm invoicebutton mr-1 mt-1" title="' . lang('invoice') . '" href="finance/invoice?id=' . $payment->id . '"><i class="fa fa-file-invoice"></i> ' . lang('invoice') . '</a>';
            $options4 = '<a class="btn btn-success btn-sm invoicebutton mr-1 mt-1" title="' . lang('print') . '" style="color: #fff;" href="finance/printInvoice?id=' . $payment->id . '"target="_blank"> <i class="fa fa-print"></i> ' . lang('print') . '</a>';
            if ($this->ion_auth->in_group(array('admin', 'Accountant'))) {
                if ($payment->payment_from == 'payment' || empty($payment->payment_from)) {
                    if (count($lab_reports_previous) == count($lab_pending) || empty($lab_reports_previous)) {
                        $options3 = '<a class="btn btn-danger btn-sm delete_button mr-1 mt-1" title="' . lang('delete') . '" href="finance/delete?id=' . $payment->id . '" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"></i> ' . lang('delete') . '</a>';
